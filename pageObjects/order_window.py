...
class orderWindowPage(BrowserUtils):
    ...
    def orderWindow(self, test_results, expected="pass"):
        ...
        def _close_order_window_if_open():
            close_xpaths = [
                "//div[@class='cursor-pointer' and descendant::svg[@stroke-width='1.5']]"
            ]
            try:
        ...
        def _scrip_by_index(i):
            xp = f"(//div[contains(@class,'border-b') and contains(@class,'bgCardColor') and contains(@id,'|NSE')])[{i}]"
            return wait.until(EC.presence_of_element_located((By.XPATH, xp)))

        def click_scrip_and_buy(i):
            ...
            # Click the row (optional)
            try:
                scrip.click()
            except:
                driver.execute_script("arguments[0].click();", scrip)

            # Now click BUY (B button)
            buy_btn = scrip.find_element(
                By.XPATH,
                ".//button[contains(@class,'positiveBackground') and normalize-space()='B']"
            )
            ...
            print(f"✔ Clicked scrip {i} and Buy button")
        ...
        #     time.sleep(0.2)
        #
        #     # Correct Buy button XPaths based on actual UI
        #     buy_xpaths = [
        #         ".//button[normalize-space()='B']",
        #         ".//button[contains(@class,'positiveBackground') and normalize-space()='B']",
        #         ".//button[contains(@class,'owreviewOrderBtn')]",  # real class
        #         ".//button[contains(@class,'owPositiveBackground')]",  # real class
        #     ]
        #
        #     buy_btn = None
        #
        #     for rel in buy_xpaths:
        #         try:
        #             buy_btn = scrip.find_element(By.XPATH, rel)
        #             break
        #         except Exception:
        #             continue
        #
        #     if not buy_btn:
        #         raise Exception("Buy button not found inside scrip row")
        #
        #     _safe_click(buy_btn)
        #     time.sleep(0.4)  # allow modal animation
        ...
        def _select_regular():
            xp = 'Regular_btn'
            el = wait.until(EC.element_to_be_clickable((By.ID, xp)))
            _safe_click(el)
            time.sleep(0.2)
        ...
        def _select_product_preferring_intraday():
            # Try NRML first; if not present, fall back to Delivery/CNC
            intraday_xpaths = [
                "//button[normalize-space()='Intraday']",
    ...
    def orderWindow_Sell(self, test_results, expected="pass"):
        ...
        sell_logged_ = False
        regular_logg_sell = False
        limit_sell = False
        advance_options_sell = False
        longterm_sell = False
        intraday_sell = False
        # buy_btton_sell = False
        stop_loss_sell = False
        pending_checkbox_sell = False
